...
test('README communicates pipeline, maturity, contribution, and ecosystem role', () => {
  ...
  const siteLink = readme.match(/\[Code Review\]\(([^)]+)\)/)
  assert.ok(siteLink, 'README must link to Code Review')
  assert.equal(new URL(siteLink[1]).origin, 'https://code-review.agentskit.io')
})
...
test('Changesets release automation is tokenless, gated, and recoverable', () => {
  ...
  assert.doesNotMatch(publishWorkflow, /push:\n    branches: \[main\]/)
  assert.doesNotMatch(publishWorkflow, /github\.event_name == 'push'/)
  ...
  assert.match(publishWorkflow, /github\.event\.pull_request\.head\.repo\.full_name == github\.repository/)
  assert.doesNotMatch(publishWorkflow, /workflow_dispatch|RECOVERY_VERSION|codex\/release-0\.4\.1/)
