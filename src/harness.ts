...
function canaryChecks(state: BatchCoverageState, artifact: BatchReviewArtifact): HarnessCheck[] {
  ...
  return [
    ...
    { id: 'manifest.packs', ok: Boolean(expected && JSON.stringify(expected.packIds) === JSON.stringify(artifact.batch.packIds) && (!expected.packIds || JSON.stringify(expected.packIds) === JSON.stringify(artifact.review.evidence.contextPacks?.map((pack) => pack.id)))), detail: 'artifact context packs match the planned batch', remediation: 'review every immutable context pack before consolidation' },
